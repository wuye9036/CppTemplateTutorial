...
这时我们就需要请出C++11标准了。这是到目前为止，我们第一次阅读标准。我希望能尽量减少直接阅读标准的次数，因此即便是极为复杂的模板匹配决议我都暂时没有引入标准中的描述。
然而，Template引入的“双阶段名称查找（Two phase name lookup）”堪称是C++中最黑暗的角落 —— 这是LLVM的团队自己在博客上说的 —— 因此在这里阅读标准是必要的。

####2.3.2 名称查找：I am who I am
在C++标准中，名称查找（name lookup）集中出现在三处。第一处是3.4节，标题名就叫“Name Lookup”；第二处在10.2节，继承关系中的名称查找；第三处在14.6节，名称解析（name resolution）
